#!/usr/bin/env Rscript
#Adapted from https://github.com/vikasgupta1812/Indeed.com-scraping-using-rVest
...
searchlimit <- if(count<10001) count else 10001

i=0
while (i<searchlimit) {
    url = "http://www.indeed.com/jobs?q=data&l=Virginia"
    start = "&start="
    inc = as.character(i)
    ...
    try({
        write.table(mylist, file="/home/dan/Desktop/data_jobs_VA.csv", row.names=FALSE, sep=",", append=TRUE, col.names=FALSE)
    })
    i <- i+10
}
